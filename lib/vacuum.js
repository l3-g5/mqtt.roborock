...
class vacuum {
	...
	async getMap(duid) {
		if (this.adapter.config.enable_map_creation == true) {
			...
			await this.rr_mqtt_connector.sendRequest(duid, "get_map_v1", [], true).then(map => {
				...
				this.adapter.log.error("Failed to create map. " + error);
	...
	async command(duid, parameter, value) {
		switch (parameter) {
			case "load_multi_map":
				await this.rr_mqtt_connector.sendRequest(duid, "load_multi_map", value).then(async (result) => {
					if (result[0] == "ok") {
					...
					this.adapter.log.warn("Failed to execute load_multi_map. " + error);
				});
				break;
				...
				await this.adapter.getStateAsync("Devices." + duid + ".deviceStatus.map_status").then(async roomFloor => {
					await this.rr_mqtt_connector.sendRequest(duid, "get_room_mapping", []).then(async mappedRoomList => {
						...
						await this.adapter.getStateAsync("Devices." + duid + ".floors.cleanCount").then(async cleanCount => {
							...
							await this.rr_mqtt_connector.sendRequest(duid, parameter, [roomList]).then(result => {
								...
								this.adapter.log.warn("Failed to execute app_segment_clean. " + error);
							});
						});
					}).catch(error => {
						this.adapter.log.warn("Failed to execute get_room_mapping. " + error);
					});
				});
				break;
			...
			case "reset_consumable":
				...
				await this.rr_mqtt_connector.sendRequest(duid, parameter, [value]).then(async parameter => {
					...
					this.adapter.log.warn("Failed to execute " + parameter + ". " + error);
				});
				break;
	...
	async getParameter(duid, parameter, attribute) {
		...
		{
			mode = parameter;
			await this.rr_mqtt_connector.sendRequest(duid, parameter, []).then(networkInfo => {
			...
			await this.rr_mqtt_connector.sendRequest(duid, "get_consumable", []).then(async consumables => {
				...
				this.adapter.log.warn("Failed to execute get_consumable. " + error);
			});
		}
		else if (parameter == "get_status") {
			await this.rr_mqtt_connector.sendRequest(duid, "get_status", []).then(deviceStatus => {
				...
				this.adapter.log.warn("Failed to execute get_status. " + error);
			});
		}
		else if (parameter == "get_room_mapping") {
			await this.rr_mqtt_connector.sendRequest(duid, "get_status", []).then(async deviceStatus => {
				const roomFloor = deviceStatus[0]["map_status"] >> 2 ?? -1; // to get the currently selected map perform bitwise right shift
				await this.rr_mqtt_connector.sendRequest(duid, "get_room_mapping", []).then(mappedRooms => {
			...
			await this.rr_mqtt_connector.sendRequest(duid, "get_multi_maps_list", []).then(mapList => {
				...
				this.adapter.log.warn("Failed to execute get_multi_map_list. " + error);
			});
		}
		else if (parameter == "get_fw_features") {
			this.adapter.log.debug("Firmware features request");
			await this.rr_mqtt_connector.sendRequest(duid, parameter, []).then(async firmwareFeatures => {
				...
				this.adapter.log.warn("Failed to execute get_fw_features. " + error);
			});
		}
			...
			this.rr_mqtt_connector.sendRequest(duid, "get_photo" , {
				"data_filter": {
					"img_id": attribute,
				...
				this.adapter.log.error("Failed photo response !!!!!!!!!!!!! " + e);
			});
		}
		else if (typeof(this.parameterFolders[parameter]) != "undefined")
		{
			mode = parameter.substring(4);
			await this.rr_mqtt_connector.sendRequest(duid, parameter, []).then(attribute_val => {
			...
			// unknown parameter
			await this.rr_mqtt_connector.sendRequest(duid, parameter, []).then(unknown_parameter_val => {
				// this.adapter.setStateAsync("Devices." + duid + "." + targetFolder + "." + mode, { val: attribute_val[0], ack: true });
	...
	async getCleanSummary(duid) {
		await this.rr_mqtt_connector.sendRequest(duid, "get_clean_summary", []).then(async cleaningAttributes => {
					...
					case "3":
						for (const cleaningRecord in cleaningAttributes[cleaningAttribute])
						{
							await this.rr_mqtt_connector.sendRequest(duid, "get_clean_record", [cleaningAttributes[cleaningAttribute][cleaningRecord]]).then(async cleaningRecordAttributes => {
								...
								this.adapter.log.warn("Failed to execute get_clean_record. " + error);
							});
						}
			...
			this.adapter.log.warn("Failed to execute get_clean_summary. " + error);
		});
	}
