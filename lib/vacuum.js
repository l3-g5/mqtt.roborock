...
const supportedRobots = {
	...
	"roborock.vacuum.a73": "roborock_vacuum_a73", // Roborock Q8 Max (Plus)
	"roborock.vacuum.a72": "roborock_vacuum_a72", // Roborock Q5 Pro
	"roborock.wm.a102": "roborock_wm_a102", // Zeo One
	"roborock.wetdryvac.a56": "roborock_wetdryvac_a56", // Dyad Pro
...
class vacuum {
	...
	async getMap(duid) {
		if (this.adapter.config.enable_map_creation) {
			...
			const connector = await this.adapter.getConnector(duid);

			try {
				// const map = await connector.sendRequest(duid, this.adapter.getRequestId(), "get_map_v1", [], true);
				const map = await this.adapter.rr_mqtt_connector.sendRequest(duid, this.adapter.getRequestId(), "get_map_v1", {}, true);
				const mappedRooms = await connector.sendRequest(duid, this.adapter.getRequestId(), "get_room_mapping", []);

				const deviceStatus = await connector.sendRequest(duid, this.adapter.getRequestId(), "get_status", []);
				...
				fs.writeFile("./test.rrmap", map, (error) => {
					if (error) this.adapter.catchError(error, "writing test.rrmap", duid);
				});
	...
	async getCleaningRecordMap(duid, startTime) {
		try {
			const cleaningRecordMap = await this.adapter.rr_mqtt_connector.sendRequest(duid, this.adapter.getRequestId(), "get_clean_record_map", { start_time: startTime }, true);
	...
	async command(duid, parameter, value) {
		...
		try {
			switch (parameter) {
				case "load_multi_map": {
					const result = await connector.sendRequest(duid, this.adapter.getRequestId(), "load_multi_map", value);
				...
				case "app_segment_clean": {
					...
					const mappedRoomList = await connector.sendRequest(duid, this.adapter.getRequestId(), "get_room_mapping", []);
					...
					const result = await connector.sendRequest(duid, this.adapter.getRequestId(), "app_segment_clean", [roomList]);
				...
				case "reset_consumable":
					await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, [value]);
					this.adapter.log.info(`Consumable ${parameter} successfully reset.`);

					break;

				case "app_set_dryer_status": {
					const result = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter);
				...
				case "app_zoned_clean": {
					const result = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, value);
				...
				case "set_water_box_distance_off": {
					...
					const result = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, parameterValue);
	...
	async getParameter(duid, parameter, attribute) {
		...
		try {
			if (targetFolder == "networkInfo") {
				mode = parameter;
				const networkInfo = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, []);
				...
				const consumables = (await connector.sendRequest(duid, this.adapter.getRequestId(), "get_consumable", []))[0];
				...
				const mapList = await connector.sendRequest(duid, this.adapter.getRequestId(), "get_multi_maps_list", []);
				...
				this.adapter.log.debug(`Firmware features request for ${duid}`);
				const firmwareFeatures = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, []);
				...
				const serverTimers = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, []);
				...
				const timers = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, []);
				...
				const photoresponse = await this.adapter.rr_mqtt_connector.sendRequest(
					duid,
				...
				const attribute_val = JSON.stringify(await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, {}));
				...
				const attribute_val = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, {});
				...
				if (mode === "water_box_custom_mode") {
					attribute_val[0] = attribute_val.water_box_mode;
				}
				if (typeof attribute_val[0] == "object") {
					attribute_val[0] = JSON.stringify(attribute_val[0]);
				}
				...
				// unknown parameter
				const unknown_parameter_val = await connector.sendRequest(duid, this.adapter.getRequestId(), parameter, []);
	...
	async getCleanSummary(duid) {
		...
		try {
			const cleaningAttributes = await connector.sendRequest(duid, this.adapter.getRequestId(), "get_clean_summary", []);
			...
			for (const cleaningAttribute in cleaningAttributes) {
				...
				if (["clean_time", "clean_area", "clean_count"].includes(mappedAttribute)) {
					...
					for (const cleaningRecord in cleaningAttributes[cleaningAttribute]) {
						...
						const cleaningRecordAttributes = (await connector.sendRequest(duid, this.adapter.getRequestId(), "get_clean_record", [cleaningRecordID]))[0];
