...
class MapCreator {
	...
	getY(dimensions, px){
		return dimensions.height - 1 - Math.floor(px * this.scaleFactor * this.scaleFactor / dimensions.width);
	}

	robotXtoPixelX(image, robotCoord)
	{
		return (robotCoord - image.position.left) * this.scaleFactor;
	}
	robotYtoPixelY(image, robotCoord)
	{
		return (image.dimensions.height/this.scaleFactor + image.position.top - robotCoord) * this.scaleFactor;
	}
	...
	canvasMap(mapdata, options) {
		...
		if (mapdata && mapdata.IMAGE && mapdata.IMAGE.pixels && mapdata.IMAGE.pixels.carpet) {
			const offset = 15 * this.scaleFactor; // why the offset??
			ctx.beginPath();
			mapdata.IMAGE.pixels.carpet.forEach(px => {
				ctx.strokeStyle = "rgba(0,0,0,0.5)";
				ctx.lineWidth = 1;

				ctx.moveTo(this.getX(mapdata.IMAGE.dimensions, px) + offset, this.getY(mapdata.IMAGE.dimensions, px));
				ctx.lineTo(this.getX(mapdata.IMAGE.dimensions, px) + offset + 4, (this.getY(mapdata.IMAGE.dimensions, px)) + 4);
			});
			ctx.stroke();
		}
		...
		// Male geplanten Pfad
		if (typeof (mapdata.GOTO_PREDICTED_PATH) !== "undefined") {
		...
		if (mapdata.VIRTUAL_WALLS) {
			mapdata.VIRTUAL_WALLS.forEach(wall => {
				const virtualWallMinX = Math.min(wall[0], wall[2]);
				const virtualWallMinY = Math.min(wall[1], wall[3]);
				const virtualWallMaxX = Math.max(wall[0], wall[2]);
				const virtualWallMaxY = Math.max(wall[1], wall[3]);

				const forbiddenSizeX = virtualWallMaxX - virtualWallMinX;
				const forbiddenSizeY = virtualWallMaxY - virtualWallMinY;

				ctx.fillStyle = "rgba(255, 0, 0, 0.5)";
				ctx.fillRect(
					this.robotXtoPixelX(mapdata.IMAGE, virtualWallMinX/50),
					this.robotYtoPixelY(mapdata.IMAGE, virtualWallMaxY/50),
					forbiddenSizeX/50 * this.scaleFactor,
					forbiddenSizeY/50 * this.scaleFactor
				);
				ctx.lineWidth = 1 * this.scaleFactor / 2;
		...
		if (mapdata.OBSTACLES3) {
			mapdata.OBSTACLES3.forEach(obstacle => {
				ctx.fillStyle = "red";
				ctx.beginPath();
				ctx.arc(
					this.robotXtoPixelX(mapdata.IMAGE, obstacle[0]/50),
					this.robotYtoPixelY(mapdata.IMAGE, obstacle[1]/50),
					5, 0, 2 * Math.PI
				);
				ctx.fill();
			});
		...
		if (mapdata.CHARGER_LOCATION) {
			if (typeof mapdata.CHARGER_LOCATION.position[0] !== "undefined" && typeof mapdata.CHARGER_LOCATION.position[1] !== "undefined") {
				ctx.beginPath();
				const img_charger_rotated = this.rotateCanvas(img_charger, mapdata.CHARGER_LOCATION.angle);
				ctx.drawImage(img_charger_rotated,
		...
		if (mapdata.ROBOT_POSITION) {
			if (mapdata.PATH.current_angle && typeof mapdata.ROBOT_POSITION[0] !== "undefined" && typeof mapdata.ROBOT_POSITION[1] !== "undefined") {
				ctx.beginPath();
				...
				ctx.drawImage(img_robot_rotated,
		...
		if (mapdata.GOTO_TARGET) {
			...
			if (typeof mapdata.GOTO_TARGET[0] !== "undefined" && typeof mapdata.GOTO_TARGET[1] !== "undefined") {
				ctx.beginPath();
				ctx.drawImage(go_to_pin_img,
		...
		ctx_trimmed.drawImage(canvas, minleft - offset, mintop - offset, maxleft - minleft + 2* offset, maxtop - mintop + 2*offset, 0, 0, maxleft - minleft + 2* offset, maxtop - mintop + 2*offset);
